...
func parsePlayersPos(vr *sheets.ValueRange, teams []*Team, row int, col int, pt models.PlayerType) ([]*Player, error) {
	...
	for pn != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("value read for player %s, in cell: %s not an integer %s", pn, indicesToCellStr(row, valCol), err.Error())
		}
		...
		for i := 0; i < len(teams); i++ {
			...
			if bs != "" {
				...
				if err != nil {
					return nil, fmt.Errorf("bid read for player %s, in cell: %s not an integer %s", pn, indicesToCellStr(row, bc), err.Error())
				}
				if bv > maxBid {
					maxBid = bv
					maxTeam = teams[i]
...
func parsePlayers(vr *sheets.ValueRange, teams []*Team) (map[models.PlayerType][]*Player, error) {
	...
	for i, pt := range models.AllPlayerTypes {
		...
		if err != nil {
			return nil, fmt.Errorf("unable to parse players for position %v in spreasheet. %s", pt, err.Error())
		}
	}
